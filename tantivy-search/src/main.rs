...
fn main() -> tantivy::Result<()> {
    ...
    let search_query = "Salinas reaching";
    let queries: Vec<_> = search_query
        .split_whitespace()
        .into_iter()
        .map(|term| -> Vec<_> {
            vec![
        ...
        .collect();
    let flat_queries: Vec<_> = queries.into_iter().flatten().collect();

    let query = BooleanQuery::new(flat_queries);
